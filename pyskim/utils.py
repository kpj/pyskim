...
def text_barchart(values: List[int], safe: bool = True) -> str:
    ...
    bar_values = pd.cut(values, bins=bins, labels=bar_characters, include_lowest=True)

    return "".join(bar_values)


def text_histogram(values: np.ndarray, bins: Union[int, List[int]] = 10) -> str:
...
if __name__ == "__main__":
    txt = text_histogram(
        pd.Series(
            np.r_[np.random.normal(10, 2, size=500), np.random.normal(50, 7, size=500)]
        )
    )
    print(txt)
