    ...
    df = pd.DataFrame(
        {
            ...
            "category_column": pd.Series(
                np.random.choice(["apple", "orange"], size=N), dtype="category"
            ),
            "string_column": pd.Series(
                np.random.choice(["foo", "bar", "baz", "qux"], size=N), dtype="string"
            ),
            "object_column": pd.Series(
                np.random.choice(["obj", 1, False], size=N), dtype="object"
            ),
